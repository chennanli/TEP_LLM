        ...
        def start_bridge():
            ...
            try:
                ...
                if 'tep_bridge' in self.bridge.processes and self.bridge.check_process_status('tep_bridge'):
                    return jsonify({'success': True, 'message': 'Bridge already running'})
                ...
                return jsonify({'success': False, 'message': f'Bridge failed: {e}'})
...
<body>
            ...
            <div class="controls-grid">
                <div class="control-card">
                    ...
                    <button class="btn btn-success" onclick="startTEP()">▶️ Start TEP Simulation</button>
                        <!-- No-JS fallbacks for TEP start/stop/restart -->
                        <div style="margin-top:6px">
                          <form method="POST" action="/api/tep/start" style="display:inline">
                            <button class="btn">(No-JS) Start TEP</button>
                          </form>
                          <form method="POST" action="/api/tep/stop" style="display:inline">
                            <button class="btn">(No-JS) Stop TEP</button>
                          </form>
                          <form method="POST" action="/api/tep/restart" style="display:inline">
                            <button class="btn">(No-JS) Restart TEP</button>
                          </form>
                        </div>
                    ...
                    <button class="btn btn-danger" onclick="stopTEP()">⏹️ Stop TEP Simulation</button>
                        <div style="margin-top:10px;">
                            ...
                            <!-- No-JS fallbacks for speed -->
                            <div style="margin-top:6px">
                                <form method="POST" action="/api/speed" style="display:inline">
                                  <input type="hidden" name="mode" value="demo"/>
                                  <button class="btn">(No-JS) Set Demo</button>
                                </form>
                                <form method="POST" action="/api/speed" style="display:inline">
                                  <input type="hidden" name="mode" value="real"/>
                                  <button class="btn">(No-JS) Set Real</button>
                                </form>
                            </div>
                ...
                <div class="control-card">
                    ...
                    <button class="btn btn-danger" onclick="stopAll()">🛑 Stop Everything</button>
                        <!-- No-JS fallback for Emergency Stop -->
                        <div style="margin-top:6px">
                          <form method="POST" action="/api/stop/all" style="display:inline">
                            <button class="btn btn-danger">(No-JS) Stop Everything</button>
                          </form>
                        </div>
    ...
    <script>
        // Global error handlers to surface JS issues in the UI
        window.onerror = function(msg, src, line, col, err){
            const s = document.getElementById('status');
            if (s){ s.style.display='block'; s.className='btn-danger'; s.textContent = `JS error: ${msg} @ ${line}:${col}`; }
            console.error('JS error', msg, src, line, col, err);
        };
        window.onunhandledrejection = function(e){
            const s = document.getElementById('status');
            const reason = e && e.reason ? (e.reason.message || e.reason) : 'unknown';
            if (s){ s.style.display='block'; s.className='btn-danger'; s.textContent = `Promise rejection: ${reason}`; }
        ...
        function startTEP() {
            console.log('startTEP() called');
            const btns = document.querySelectorAll("button[onclick*='startTEP']");
            console.log('Found TEP buttons:', btns.length);
            btns.forEach(b=>b && (b.disabled=true));

            fetch('/api/tep/start', {method: 'POST'})
                .then(response => {
                    console.log('TEP start response:', response.status);
                    return response.json();
                })
                .then(data => {
                    console.log('TEP start data:', data);
                ...
                .catch(e => {
                    console.error('TEP start error:', e);
        ...
        function stopTEP() {
            const btns = document.querySelectorAll("button[onclick*='stopTEP']");
            btns.forEach(b=>b && (b.disabled=true));
            fetch('/api/tep/stop', {method: 'POST'})
                ...
                .catch(e=>showMessage(`Stop TEP failed: ${e}`,'error'))
                .finally(()=>btns.forEach(b=>b && (b.disabled=false)));
        }
        ...
        function startBackend() {
            ...
            fetch('/api/faultexplainer/backend/start', {method: 'POST'})
                .then(response => {
                    console.log('Backend start response:', response.status);
                    return response.json();
                })
                .then(data => {
                    console.log('Backend start data:', data);
                    showMessage(data.message, data.success ? 'success' : 'error');
                    if (data.success) {
                        btns.forEach(b=>{
                            b.classList.add('btn-success');
        ...
        function startBridge(){
            fetch('/api/bridge/start', {method: 'POST'})
              ...
              .catch(e=>showMessage(`Bridge start failed: ${e}`, 'error'));
        }
        function stopBridge(){
        ...
        function startFrontend() {
            const btns = document.querySelectorAll("button[onclick*='startFrontend']");
            btns.forEach(b=>b && (b.disabled=true));
            fetch('/api/faultexplainer/frontend/start', {method: 'POST'})
                ...
                .catch(e => showMessage(`Frontend start failed: ${e}`, 'error'))
                .finally(()=>btns.forEach(b=>b && (b.disabled=false)));
        }
        ...
        function setSpeed(mode) {
            ...
            .then(data => {
                ...
                const di = document.getElementById('demo-interval'); if (di) di.textContent = data.step_interval_seconds;
                const ds = document.getElementById('demo-interval-slider'); if (ds && data.mode==='demo') ds.value = data.step_interval_seconds;
            ...
            .catch(e => showMessage(`Speed update failed: ${e}`, 'error'));
        }
        ...
        function setPreset(mode){
            ...
            const cfg = mode==='demo' ? demo : (mode==='balanced' ? balanced : real);
            fetch('/api/backend/config/runtime',{
                method:'POST', headers:{'Content-Type':'application/json'},
                body: JSON.stringify({...cfg, preset: mode})
            })
            .then(r=>r.json())
            .then(data=>{
                showMessage(`Backend runtime config updated: ${JSON.stringify(data.updated)}`,'success');
                const label = mode==='demo' ? 'Demo' : (mode==='balanced' ? 'Balanced' : 'Realistic');
                document.getElementById('preset-mode').textContent = label;
                document.getElementById('btn-preset-demo').classList.toggle('btn-active', mode==='demo');
                const bp = document.getElementById('btn-preset-balanced'); if (bp) bp.classList.toggle('btn-active', mode==='balanced');
                document.getElementById('btn-preset-real').classList.toggle('btn-active', mode==='real');
            })
            .catch(e=>showMessage(`Config update failed: ${e}`,'error'));
        }
        ...
        function setDemoInterval(sec){
            ...
            fetch('/api/speed',{
                method:'POST', headers:{'Content-Type':'application/json'},
                body: JSON.stringify({mode:'demo', seconds})
            })
            .then(r=>r.json())
            .then(d=>{
            ...
            .catch(e=>showMessage(`Failed to set interval: ${e}`,'error'));
        }
        ...
        function setLLMInterval(sec){
            ...
            .then(r=>r.json())
            .then(_=>{
                const lab = document.getElementById('llm-interval-label'); if (lab) lab.textContent = seconds;
                showMessage(`LLM refresh interval set to ${seconds}s`, 'success');
            })
            .catch(e=>showMessage(`Failed to set LLM interval: ${e}`,'error'));
        }
