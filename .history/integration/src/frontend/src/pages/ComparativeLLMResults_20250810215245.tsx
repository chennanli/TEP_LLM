...
export default function ComparativeLLMResults({ results, isLoading }: ComparativeLLMResultsProps) {
  ...
  console.log('ComparativeLLMResults rendered - isFrozen:', isFrozen, 'results:', !!results);
        ...
        <Paper p="md" bg="blue.0">
          <Group justify="space-between">
            ...
            <Group>
              ...
              <Button
                size="sm"
                variant={isFrozen ? "filled" : "light"}
                color={isFrozen ? "orange" : "blue"}
                leftSection={isFrozen ? <IconLock size={16} /> : <IconLockOpen size={16} />}
                onClick={() => setIsFrozen(!isFrozen)}
              >
                {isFrozen ? "🔒 Display Frozen" : "🔄 Live Updates"}
              </Button>
        ...
        <SimpleGrid cols={{ base: 1, md: 2, lg: models.length > 2 ? 3 : 2 }}>
          {models.map((modelName) => (
            <ModelCard
              key={modelName}
              modelName={modelName}
              analysis={displayResults.llm_analyses[modelName]}
